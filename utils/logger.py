...
class board(object):
    ...
    ''' 1. loss curve '''
    def add_projection_loss(self, loss, step):
        self.writer.add_scalar('Loss/projection', loss, step)
    ...
    def add_dense_matching_loss(self, loss, step):
        self.writer.add_scalar('Loss/dense_matching', loss, step)

    ''' 2. result image '''
    def add_image(self, image0, image1, step):
        self.writer.add_image('Image/0', image0, step)
        self.writer.add_image('Image/1', image1, step)

    def add_keypoint_image(self, keypoint0, keypoint1, step):
        self.writer.add_image('Keypoint/0', keypoint0, step)
        self.writer.add_image('Keypoint/1', keypoint1, step)

    def add_score_map(self, score_map0, score_map1, step):
        self.writer.add_image('Score_map/0', score_map0, step)
        self.writer.add_image('Score_map/1', score_map1, step)

    def add_local_desc_map(self, local_desc_map0, local_desc_map1, step):
        self.writer.add_image('Local_desc_map/0', local_desc_map0, step)
        self.writer.add_image('Local_desc_map/1', local_desc_map1, step)
